/*
  ...
  3. Sample Data
    - 26 sample products across all categories
    - Multiple variants per product
    - Sample images for all products

  4. Functions and Triggers
    - Helper function for user email detection
    - Auto-update timestamps on product changes
*/
...
-- Insert default variants for all products
DO $$
DECLARE
...
BEGIN
    FOR product_record IN SELECT * FROM products LOOP
        -- Determine size and color based on category and subcategory
        IF product_record.category = 'apparel' THEN
            CASE 
                WHEN product_record.subcategory = 'kids-wear' THEN
                    variant_size := '2-3Y';
                    variant_color := 'Pink';
                WHEN product_record.subcategory IN ('saree', 'dupatta') THEN
                ...
                ELSE
                    ...
                    variant_color := 'Red';
            END CASE;
        ELSIF product_record.category = 'jewelry' THEN
            CASE 
                WHEN product_record.subcategory = 'necklace' THEN
                    variant_size := '18"';
                WHEN product_record.subcategory = 'ring' THEN
                    variant_size := '7';
                WHEN product_record.subcategory = 'bracelet' THEN
                    variant_size := '7"';
                WHEN product_record.subcategory = 'anklet' THEN
                    variant_size := '9"';
                ELSE
                    variant_size := 'One Size';
            END CASE;
        ...
        -- Create variant name
        IF variant_color IS NOT NULL THEN
            variant_name := variant_size || ' - ' || variant_color;
        ELSE
            variant_name := variant_size;
        END IF;
...
-- Insert additional variants for apparel products (L - Blue)
...
WHERE p.category = 'apparel' 
  AND p.subcategory NOT IN ('saree', 'dupatta', 'kids-wear')
LIMIT 5;

-- Insert additional variants for jewelry (Rose Gold)
INSERT INTO product_variants (product_id, variant_name, size, color, price, stock, is_default)
SELECT 
    p.id,
    CASE 
        WHEN p.subcategory = 'necklace' THEN '20" - Rose Gold'
        WHEN p.subcategory = 'ring' THEN '8 - Rose Gold'
        WHEN p.subcategory = 'bracelet' THEN '7.5" - Rose Gold'
        ELSE 'One Size - Rose Gold'
    END,
    CASE 
...
-- Insert primary images for all products
...
-- Insert secondary images for featured products
INSERT INTO product_images (product_id, image_url, alt_text, display_order, is_primary)
SELECT 
    p.id,
    CASE 
