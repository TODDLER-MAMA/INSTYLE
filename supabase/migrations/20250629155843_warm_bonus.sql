/*
  # Enhanced Products Schema with Variants and Images

  1. New Tables
    - `products` - Main product information
      - `id` (uuid, primary key)
      - `name` (text, required)
      - `category` (text, required, enum: apparel/jewelry/beauty)
      - `subcategory` (text, required)
    ...
    - `product_variants` - Product variations (size, color, etc.)
      ...
      - `size` (text, optional)
      - `color` (text, optional)
      - `material_variant` (text, optional)
  ...
  2. Security
    ...
    - Public can read active products, variants, and images
    - Only admin can manage all product data
    - Updated policies for orders and profiles

  3. Sample Data
    - Sample products across all categories
    - Product variants with different sizes/colors
