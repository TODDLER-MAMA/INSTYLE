/*
  # Enhanced Product Catalog Migration
  ...
  3. Sample Data
    - 27 products across apparel, jewelry, and beauty categories
    - Multiple variants per product with realistic pricing
    - Product images using valid Pexels URLs
*/
...
-- Insert default product variants for each product
DO $$
DECLARE
...
BEGIN
    FOR product_record IN SELECT * FROM products LOOP
        ...
        IF product_record.category = 'apparel' THEN
            IF product_record.subcategory = 'kids-wear' THEN
                variant_size := '2-3Y';
                variant_color := 'Pink';
            ELSIF product_record.subcategory IN ('saree', 'dupatta') THEN
        ...
        ELSIF product_record.category = 'jewelry' THEN
            IF product_record.subcategory = 'necklace' THEN
                variant_size := '18"';
            ELSIF product_record.subcategory = 'ring' THEN
                variant_size := '7';
            ELSIF product_record.subcategory = 'bracelet' THEN
                variant_size := '7"';
            ELSIF product_record.subcategory = 'anklet' THEN
                variant_size := '9"';
            ELSE
                variant_size := 'One Size';
            END IF;
...
-- Insert additional variants for apparel products (excluding one-size items)
INSERT INTO product_variants (product_id, variant_name, size, color, price, stock, is_default)
SELECT 
    p.id,
    'L - Blue',
