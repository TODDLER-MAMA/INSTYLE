/*
  # Enhanced Product System with Variants and Images

  1. New Tables
    - Enhanced `products` table with better structure
    - `product_variants` table for different sizes, colors, materials, prices
    - `product_images` table for multiple images per product

  2. Features
    - Multiple product variants with individual pricing and stock
    - Up to 5 images per product with file upload support
  ...
  Note: Admin user must be created manually through Supabase Dashboard
...
CREATE POLICY "Only admin can manage products"
  ...
  FOR ALL
  TO authenticated
  USING ((jwt() ->> 'email'::text) = 'instylebd86@gmail.com'::text)
  WITH CHECK ((jwt() ->> 'email'::text) = 'instylebd86@gmail.com'::text);
